...
- **Theme đêm.** Nền tối, 12 màu vùng rực, không còn dấu ✕: ô người chơi loại trừ thì **tắt đèn** (mờ đi), càng chơi bàn cờ càng sáng dần về đáp án. Ba thẻ luật bỏ hẳn, màn chơi không nhắc luật nữa; bài hướng dẫn mở lại từ Cài đặt → Cách chơi.
- **Câu chuyện theo chương.** Mỗi màn là một *đêm gác*, năm đêm là một *chương*, mỗi chương giữ an toàn một *buồng* của tổ, hết năm buồng là xuống *tầng* mới — và tầng mới có năm buồng khác hẳn, cả tên lẫn hình. Ba tầng đầu: mặt đất (Cổng tổ, Buồng trứng, Vườn nấm, Kho kẹo, Buồng chúa), sâu hơn (Nhà trẻ, Giếng nước, Kho hạt, Chuồng rệp, Buồng trú đông), tận đáy (Tầng rễ, Mạch nước ấm, Hang lấp lánh, Kho lớn, Tim tổ); từ tầng 4 quay vòng lại, số tầng vẫn tăng. Thêm tầng thì nối một mảng khoá vào `FLOORS` trong [`src/nest.js`](src/nest.js) và năm tên vào `rooms` trong `data/i18n/*.json`. Xong đêm thứ 5 thì hộp thoại thắng đổi thành "Buồng X đã an toàn" và nút "Về tổ" đưa về trang chủ — ở đó là mặt cắt tổ ([`src/nest.js`](src/nest.js)), buồng đã giữ được treo đèn, buồng đang gác viền sáng, buồng chưa tới thì hình mờ đi. Trang chủ không có dòng chữ nào ngoài tên năm buồng.

  Cả nền lẫn hình buồng đều là **tranh vẽ**, không còn hình khối CSS/SVG nào:

  - `assets/nest-bg.png` — mặt cắt đất 3:4, có sẵn mặt cỏ, trục hầm giữa và năm hốc buồng để trống. Dựng bằng [`tools/prep_nest_bg.py`](tools/prep_nest_bg.py) (hạ về 720px, ép 96 màu, 176 KB). Tâm năm hốc đo bằng [`tools/measure_nest.py`](tools/measure_nest.py) — nó dò vùng vừa tối vừa **mịn**, vì lọc theo màu thôi thì đất ở đáy tranh cũng tối ngang hốc — rồi in ra đúng bảng `ROOM_AT` để dán vào [`src/nest.js`](src/nest.js). Đổi tranh nền thì chạy lại cả hai.
  - `assets/room-*.png` — 15 hình buồng cộng cây đèn, cắt từ một tấm 4×4 bằng [`tools/slice_rooms.py`](tools/slice_rooms.py): xén sát viền theo alpha, đệm vuông, hạ về 192px, ép 128 màu (16 file, 164 KB).

  Đường dẫn ảnh phải viết thẳng từng cái trong `ICON_URL` chứ không ghép chuỗi lúc chạy, vì [`tools/build_single.mjs`](tools/build_single.mjs) nhúng ảnh bằng cách dò chuỗi trong mã nguồn.
